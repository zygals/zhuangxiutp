...
class CartGood extends model {
    ...
    public static function getGoodsByShop($shop_id) {
        $list_ = self::where(['cart_good.shop_id'=>$shop_id,'cart_good.st'=>1,'good.st'=>1])->join('good','cart_good.good_id=good.id')->field('cart_good.id cart_good_id,cart_id,cart_good.shop_id,good_id,num,good.name good_name,good.img,good.price')->select();
        return $list_;
    }
